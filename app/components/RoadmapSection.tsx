...
const RoadmapCard = ({ item, index }: { item: RoadmapItem; index: number }) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`relative bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border ${getBorderColor()} group`}
    >
      {/* Timeline connector */}
      {index < roadmapData.length - 1 && (
        <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-px h-8 bg-gradient-to-b from-amber-300 to-transparent hidden lg:block"></div>
      )}
...
export default function RoadmapSection() {
  return (
    <section className="py-20 bg-gradient-to-br from-gray-50 via-white to-amber-50/30 relative z-10 overflow-hidden">
      <div className="container mx-auto px-4 relative">
        {/* Background decorations */}
        <div className="absolute top-10 right-10 w-32 h-32 bg-amber-200 rounded-full opacity-10 animate-pulse"></div>
        <div className="absolute bottom-10 left-10 w-40 h-40 bg-yellow-200 rounded-full opacity-10 animate-pulse delay-1000"></div>
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute left-1/2 transform -translate-x-1/2 w-px h-full bg-gradient-to-b from-amber-300 via-yellow-300 to-amber-300 opacity-30"></div>
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-100 to-yellow-100 rounded-full text-amber-700 text-sm font-medium">
