    ...
    const abi = CONTRACT_ABIS.MOCK_USDT;
    const address = CONTRACT_ADDRESS.MOCK_USDT as `0x${string}`;

    export function useMockUSDTContract() {
    ...
    const useDecimals = () =>
        useReadContract({ abi, address, functionName: "decimals" });
